...
The optional `table_prefix` argument changes the Rudel portion of standalone
runtime table names per connection:
...
That resolves tables such as `environments` to
...
Relevant create options include:
...
`content_exclude` is for the narrow case where one top-level `wp-content`
entry is already supplied outside the environment by another runtime layer.
For example, a downstream product can keep `clone_plugins=true` while
explicitly skipping one globally injected plugin:
...
App creation accepts the same content-layout options as sandbox creation,
including `shared_plugins` and `shared_uploads`.
...
App-derived sandboxes inherit the app's shared plugins/uploads policy unless
the create options override it explicitly.

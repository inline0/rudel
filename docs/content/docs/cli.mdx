...
```bash
wp rudel create [--name=<name>] [--git=<remote>] [--template=<template>] [--clone-db] [--clone-themes] [--clone-plugins] [--clone-uploads] [--clone-all] [--clone-from=<id>] [--shared-plugins] [--shared-uploads] [--owner=<owner>] [--labels=<labels>] [--purpose=<purpose>] [--protected] [--ttl-days=<days>] [--expires-at=<timestamp>] [--type=<type>]
```
...
`--shared-plugins` and `--shared-uploads` are opt-in exceptions to the default fully local content model. They keep the environment-local `wp-content` root, but replace `plugins` and/or `uploads` with links back to the host directories.
...
```bash
wp rudel app create --domain=<domain> [--name=<name>] [--clone-db] [--clone-themes] [--clone-plugins] [--clone-uploads] [--clone-all] [--clone-from=<id>] [--shared-plugins] [--shared-uploads] [--git=<remote>] [--branch=<branch>] [--dir=<dir>] [--owner=<owner>] [--labels=<labels>] [--purpose=<purpose>] [--protected] [--ttl-days=<days>] [--expires-at=<timestamp>]
```
...
```bash
wp rudel app create-sandbox <id> [--name=<name>] [--shared-plugins] [--shared-uploads] [--owner=<owner>] [--labels=<labels>] [--purpose=<purpose>] [--protected] [--ttl-days=<days>] [--expires-at=<timestamp>]
```

If the source app tracks a Git remote, branch, and directory, the derived sandbox inherits that metadata automatically.

If the source app uses shared plugins or uploads, the derived sandbox inherits those settings automatically unless you override them with the sandbox creation flags.

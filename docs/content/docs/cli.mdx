...
```bash
wp rudel create [--name=<name>] [--github=<repo>] [--engine=<engine>] [--template=<template>] [--clone-db] [--clone-themes] [--clone-plugins] [--clone-uploads] [--clone-all] [--clone-from=<id>]
```
...
```bash
wp rudel app create --domain=<domain> [--name=<name>] [--engine=<engine>] [--clone-all]
```

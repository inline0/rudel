...
Rudel creates sandboxes as multisite sites. There is no engine choice anymore, only different ways to seed the initial content and metadata.
...
GitHub commands are optional, but they become important once a sandbox is carrying real code changes in worktree-managed themes or plugins.
...
Use `--github`, `--branch`, and `--dir` when the app should carry one tracked code source. That tracked source becomes the default GitHub context for app-derived sandboxes and deployment history.

The `--domain` value is also the app's canonical Rudel URL. `wp rudel app info` and app deploy history report that primary domain even though the backing multisite site still exists underneath.
...
This is also where you change or clear the app's tracked GitHub source after creation.
...
If the source app tracks a GitHub repository, branch, and directory, the derived sandbox inherits that metadata automatically.

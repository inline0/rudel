...
Create a sandbox. This is the command you use when you want a new disposable overlay environment for change work.

```bash
wp rudel create [--name=<name>] [--theme=<slug>] [--git=<remote>] [--template=<template>] [--clone-from=<id>] [--owner=<owner>] [--labels=<labels>] [--purpose=<purpose>] [--protected] [--ttl-days=<days>] [--expires-at=<timestamp>] [--type=<type>]
```

Rudel creates sandboxes as overlays with cloned WordPress tables and a copied active theme. Plugins, uploads, and users are shared with the host by default.

### `wp rudel list`

List sandboxes. This is the operator view of the Rudel environment registry.
...
Show sandbox details, including lifecycle metadata and Rudel-specific overlay context.
...
Destroy a sandbox permanently. Rudel removes the environment record, generated directory, and cloned environment tables.
...
Everything under `wp rudel app ...` is about the long-lived side of the system. Apps use the same overlay runtime as sandboxes, but they accumulate operational metadata that sandboxes usually do not keep for long.
...
```bash
wp rudel app create --domain=<domain> [--name=<name>] [--theme=<slug>] [--clone-from=<id>] [--git=<remote>] [--branch=<branch>] [--dir=<dir>] [--owner=<owner>] [--labels=<labels>] [--purpose=<purpose>] [--protected] [--ttl-days=<days>] [--expires-at=<timestamp>]
```
...
The `--domain` value is also the app's canonical Rudel URL. `wp rudel app info` and app deploy history report that primary domain. On local networks that run on a non-default port, the rendered canonical URL keeps that port as part of the reported app URL.
...
Plugins and uploads are shared in the current overlay runtime. The app-derived sandbox gets its own cloned table prefix and copied active theme, while plugin files and uploaded files continue to come from the host WordPress installation.

If the source app has a primary domain, Rudel rewrites cloned site state from that app domain into the sandbox URL so local change work stays isolated. Deploying back later replaces the app's overlay state, not the host site.

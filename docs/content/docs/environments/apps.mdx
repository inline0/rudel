...
Apps require a primary domain value at creation time, and Rudel stores that metadata in its app tables. Additional domains are also metadata, not a second routing system. WordPress serves the app as a normal site, and Rudel keeps track of the domain relationships that matter to operators and higher-level tooling.

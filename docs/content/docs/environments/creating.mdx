...
Those options compose in the way you would expect. Templates and GitHub seeding help you start from a known shape, while clone flags decide how much host or source-environment state should be copied into the new environment-local `wp-content` and site tables.

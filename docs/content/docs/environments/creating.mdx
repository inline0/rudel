...
description: All the ways to create a sandbox or app, from blank to fully cloned.
---
...
## Cloning from another environment

You can also create a new sandbox by copying an existing sandbox or app:

```bash
wp rudel create --name="copy" --clone-from=my-sandbox-a1b2
```

This duplicates the source environment's database and `wp-content` directory, then rewrites all URLs and table prefixes for the new sandbox. The source and target must use the same engine.

...
When you visit a prefixed sandbox or app preview URL in the browser, Rudel automatically sets a `rudel_sandbox` cookie scoped to that preview path. That lets direct entrypoints such as `wp-login.php` and `wp-admin/` stay inside the same preview flow without leaking preview state onto the host root or host admin. To return to the host, append `?adminExit` to any prefixed preview URL.

...
```text
wp-content/{profile_environments_dir}/{environment_id}/themes/{theme_slug}
```
...
HTTP requests can enter a sandbox through the trusted header or cookie configured by the runtime profile.

WP-CLI commands can enter a sandbox through the environment variable configured by the runtime profile.
...
```text
{wordpress_prefix}{profile_environments_table}
{wordpress_prefix}{profile_worktrees_table}
```

...
If you use Rudel's [git integration](/docs/features/git), sandboxes can be cleaned up automatically when their tracked branches are merged. This is separate from age-based cleanup:

```bash
wp rudel cleanup --merged
```

This checks each sandbox that has local git worktrees or tracked GitHub branch metadata. If all of that sandbox's tracked branches have been merged into the base branch, the sandbox is destroyed and Rudel removes the corresponding branch state as part of cleanup.
...
This is the cleanest way to manage sandbox lifecycles when working with git or GitHub-backed code flows: create a sandbox, do the work, merge the branch or PR, and let cleanup remove the environment once that branch has clearly served its purpose.

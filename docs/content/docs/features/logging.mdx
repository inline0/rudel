...
Every sandbox has debug logging enabled by default. Since sandboxes are change environments, there is very little value in hiding errors and a lot of value in capturing them cleanly.
...
The log file is created automatically the first time WordPress encounters an error. There is no separate logging bootstrap to maintain.
...
The log file lives at `{sandbox}/wp-content/debug.log`. If you prefer to work with it directly, you can `cat`, `grep`, or open it in any editor. The `wp rudel logs` command exists because log access is part of the normal sandbox workflow, not a rare escape hatch.

Since sandboxes set `WP_DEBUG_DISPLAY` to `false`, errors do not spill into the frontend even when debug mode is on. That makes it safe to leave logging enabled permanently, which is the intended behavior for disposable environments.

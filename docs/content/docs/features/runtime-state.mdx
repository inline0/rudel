...
uploads inside that environment.
...
Another detail matters for multisite lookups: Rudel stores host-only multisite domains in the native site tables. Custom network ports belong in rendered URLs such as `WP_HOME`, not in `wp_site.domain` or `wp_blogs.domain`.

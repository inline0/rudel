...
By default, Rudel creates a small set of runtime tables:
...
Advanced embedded installs can change only the Rudel portion after the WordPress DB prefix:

- `RUDEL_RUNTIME_TABLE_PREFIX` changes the shared base name, so `$wpdb->base_prefix = 'wp_'` and `RUDEL_RUNTIME_TABLE_PREFIX = 'themeworkspace_'` produce tables like `wp_themeworkspace_environments`
...
Explicit per-table constants win over the shared `RUDEL_RUNTIME_TABLE_PREFIX`. Define any of these before Rudel installs or bootstraps persisted runtime tables. The WordPress DB prefix itself is never replaced.

...
Every Rudel environment has a real `blog_id`, a real URL, normal `wp-admin`, normal login, normal REST requests, and normal asset handling. Rudel uses that runtime contract everywhere, then layers creation, cloning, templates, backups, deploy history, worktrees, and lifecycle policy on top of it.

The product model is simple: multisite owns the runtime, and Rudel owns the environment lifecycle around that runtime.
...
Use WordPress for browser/runtime truth and Rudel for lifecycle truth. WordPress answers where a site lives and how it boots. Rudel answers who owns it, what app it came from, which worktree belongs to it, and which backup belongs to which deployment.
...
Rudel gives those sites an operator model.

...
That sentence is the foundation of the entire product. Every Rudel environment has a real `blog_id`, a real URL, normal `wp-admin`, normal login, normal REST requests, and normal asset handling. Rudel uses that browser/runtime contract everywhere, then layers creation, cloning, templates, backups, deploy history, worktrees, and lifecycle policy on top of it.

Once you see Rudel as "multisite runtime plus operator workflow", the rest of the model becomes much easier to reason about.
...
That is why Rudel feels different from plain multisite. It gives those sites an operator model.

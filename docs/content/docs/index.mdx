...
By default, sandboxes use MySQL with an isolated table prefix in the same database. Optionally, sandboxes can use SQLite for portable, file-based isolation via `--engine=sqlite`, or multisite sub-sites via `--engine=subsite` on multisite installations.
...
- **Three engines**: MySQL (default), SQLite for portable isolation, or multisite sub-site for network environments.

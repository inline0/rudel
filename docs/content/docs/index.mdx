...
normal login, normal REST requests, and normal asset handling. Each one also
has its own cloned `wp-content`, which Rudel treats as the canonical file and
code tree for that environment. Rudel uses that site identity plus that local
content tree as the base for everything else it does: creation, cloning,
...
**WordPress multisite sites** are the routing and database truth. Every Rudel environment is backed by a real multisite site with native tables, site options, uploads, admin behavior, and browser routing.
...
uploads belong to that environment. Rudel answers who owns it, what app it
came from, which worktree belongs to it, and which backup belongs to which
deployment.

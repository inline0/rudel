...
This copies the source sandbox's SQLite database and wp-content directory into a new sandbox. The `--clone-from` flag is mutually exclusive with `--clone-db` and `--clone-all`.

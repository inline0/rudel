...
class GitIntegration {
	...
	public function create_worktree( string $repo_path, string $target_path, string $branch ): bool {
		...
		exec(
			"git -C {$escaped_repo} worktree add -b {$escaped_branch} {$escaped_target} HEAD 2>&1",
			$output,
			$exit_code
		);
	...
	public function remove_worktree( string $repo_path, string $target_path ): bool {
		...
		exec( "git -C {$escaped_repo} worktree remove {$escaped_target} --force 2>&1", $output, $exit_code );
	...
	public function is_branch_merged( string $repo_path, string $branch, string $target_branch = 'main' ): bool {
		...
		exec( "git -C {$escaped_repo} branch --merged {$escaped_target} 2>&1", $output, $exit_code );
	...
	public function delete_branch( string $repo_path, string $branch ): bool {
		...
		exec( "git -C {$escaped_repo} branch -d {$escaped_branch} 2>&1", $output, $exit_code );
	...
	public function get_default_branch( string $repo_path ): string {
		...
		exec( "git -C {$escaped} symbolic-ref refs/remotes/origin/HEAD 2>&1", $output, $exit_code );
